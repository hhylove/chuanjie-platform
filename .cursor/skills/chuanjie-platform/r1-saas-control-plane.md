...
为避免把外部认证、平台账号和租户会话一次耦合，Task 5 按以下三批交付：

1. **协议入口：** 使用 Spring Security OAuth2 Client 标准实现登录重定向和回调前置校验；强制 Authorization Code、state、OIDC nonce 与 PKCE S256；未配置提供方时返回明确不可用状态，不伪造本地账号密码登录。
2. **身份映射：** 回调成功后只依据已验证的 `(issuer, subject)` 绑定外部身份；必要声明脱敏后落库，联系方式冲突进入人工处理，不静默合并账号。
3. **平台会话与租户切换：** 外部令牌不直接作为业务令牌；列出有效成员租户，选择租户后签发短期平台令牌和可轮换刷新会话，并在服务端复核成员、租户和路由版本。

首批单实例使用 Spring Security 默认 HTTP Session 暂存一次性授权请求；它不是多实例最终存储。进入水平扩容验收前必须切换到 Valkey 支撑的 Spring Session，共享 state、nonce 和 PKCE verifier；不得用负载均衡粘性会话代替共享状态。登录入口不接受调用方提供的任意回跳地址，防止开放重定向。
